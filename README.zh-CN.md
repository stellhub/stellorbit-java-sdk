...
对每个应用和规则类型保持稳定，例如 `stellorbit.payment-service.route`。每个配置值都是一个聚合 payload，包含发布元数据、用于服务端校验的类型 payload，以及 `rules` 数组。

SDK 要求配置内容使用该聚合 schema，并会把 `rules[]` 展开成本地的多条
`GovernanceRule`。
